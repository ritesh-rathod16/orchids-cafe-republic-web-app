...
export default function AdminOverview() {
  ...
  useEffect(() => {
    ...
    const ordersSubscription = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'orders' }, (payload) => {
        ...
        addNotification({
          id: newOrder.id,
          message: `New order from Table ${newOrder.table_number}`,
          type: 'new_order',
          time: new Date(),
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'orders' }, (payload) => {
        ...
        if (updatedOrder.status === 'ready') {
          addNotification({
            id: updatedOrder.id,
            message: `Order for Table ${updatedOrder.table_number} is ready`,
            type: 'order_ready',
            time: new Date(),
          ...
          addNotification({
            id: updatedOrder.id,
            message: `Table ${updatedOrder.table_number} order completed`,
  ...
  const addNotification = (notification: Notification) => {
    ...
    toast.info(notification.message, {
      icon: notification.type === 'new_order' ? '🆕' : notification.type === 'order_ready' ? '✅' : '🎉'
    });
  };
          ...
          <AnimatePresence>
            {showNotifications && (
              <motion.div
                ...
                className="absolute right-0 top-12 z-50 w-80 rounded-2xl bg-white border shadow-xl overflow-hidden"
              >
                ...
                <div className="max-h-80 overflow-y-auto">
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((notif, idx) => (
                      <div key={idx} className="p-3 border-b last:border-0 hover:bg-muted/30 transition-colors">
                        <div className="flex items-start gap-3">
                          <div className={cn(
                            "rounded-full p-2 mt-0.5",
                            notif.type === 'new_order' ? "bg-blue-100 text-blue-600" :
                            notif.type === 'order_ready' ? "bg-green-100 text-green-600" :
                            "bg-gray-100 text-gray-600"
                          )}>
                            {notif.type === 'new_order' ? <ShoppingBag className="h-3 w-3" /> :
                             notif.type === 'order_ready' ? <CheckCircle2 className="h-3 w-3" /> :
                             <DollarSign className="h-3 w-3" />}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium">{notif.message}</p>
                            <p className="text-xs text-muted-foreground">
                              {getTimeAgo(notif.time.toISOString())}
                            </p>
                          </div>
                        </div>
                      </div>
