            ...
            <tbody className="divide-y">
              {completedOrders.slice(0, 10).map((order) => (
                <tr key={order.id} className="group hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-6 py-4 text-xs text-muted-foreground">{new Date(order.created_at).toLocaleTimeString()}</td>
      ...
      <Dialog open={!!viewOrderId} onOpenChange={() => setViewOrderId(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Order placed on {viewOrder ? new Date(viewOrder.created_at).toLocaleString() : ''}
            </DialogDescription>
          </DialogHeader>
