...
export default function AdminReports() {
  ...
  const exportToCSV = () => {
    ...
    const rows = [
      ...
      ["Average Prep Time", `${reportData.avgPrepTime.toFixed(0)} mins`],
      ["CGST Collected", `₹${reportData.cgst.toFixed(2)}`],
      ["SGST Collected", `₹${reportData.sgst.toFixed(2)}`],
      ["Best Selling Item", reportData.bestSellingItem],
      ["Revenue Lost (Cancellations)", `₹${reportData.revenueLost.toFixed(2)}`]
    ];
    ...
    link.setAttribute("download", `Cafe_Report_${timeRange}_${new Date().toLocaleDateString()}.csv`);
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      {/* 2. SALES ANALYTICS & PAYMENT BREAKDOWN */}
      ...
      {/* 5. INVENTORY & CUSTOMER INSIGHTS (Simplified) */}
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card className="border-2">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
